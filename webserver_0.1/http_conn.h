...
public:
    ...
    */
    ...
    // 从状态机的状态（行的读取状态）
    // LINE_OK: 读取到一个完整的行 LINE_BAD: 行出错 LINE_OPEN: 行数据尚且不完整
...
public:
    // 初始化新接受的连接
    void init(int sockfd, const sockaddr_in &addr, bool enable_et);
    // 关闭连接
...
private:
    ...
    bool add_status_line(int statues, const char *title);
...
private:
    ...
    // 采用writev来执行写操作，故定义下面两个成员，其中m_iv_count表示被写内存块的数量
    struct iovec m_iv[2];
    int m_iv_count;
    // // 是否启用POST
    // int cgi;
